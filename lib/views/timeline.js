...
class TimelineView {
  init() {
    ...
    this.view.webContents.setWindowOpenHandler((details) => {
      link.openWithExternalBrowser(details.url).catch(log.error);
      return {action: 'deny'};
    });
    this.view.webContents.on('will-navigate', (e, url) => {
      if (!link.isTwitter(url)) {
        e.preventDefault();
        link.openWithExternalBrowser(url).catch(log.error);
      }
    });
    ...
    contextMenu({
      ...
      menu: (defaultActions, params, _browserWindow, dictionarySuggestions) => [
        ...
        defaultActions.separator(),
        {
          ...
          click: () => {
            link.openWithExternalBrowser(params.srcURL).catch(log.error);
          }
        },
        {
  ...
  loadHomePage() {
    this.loadURL(`${baseURL}/home`);
  }

  loadSearchPage(keyword) {
    this.loadURL(`${baseURL}/search?q=${keyword}&f=live`);
  }

  loadInternalLink(url) {
    this.loadXPage(`${baseURL}${url}`);
  }
  ...
  loadUserPage(id) {
    ...
    this.loadURL(`${baseURL}/${id}`);
  }

  loadLogoutPage() {
    this.loadURL(`${baseURL}/logout`);
  }
